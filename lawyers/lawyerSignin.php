...
if (isset($_POST['button'])) {
    ...
    $query = "SELECT * FROM `lawyer` WHERE u_email='$email' AND u_pass='$password'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $_SESSION['lawyer_name'] = $row['u_name'];
        $_SESSION['lawyer_id'] = $row['u_id'];
...
<body class="bg-gradient-to-r from-blue-100 via-indigo-100 to-purple-100 min-h-screen flex items-center justify-center px-4">
  ...
  <div class="w-full max-w-md bg-white shadow-2xl rounded-xl p-8 animate-fade-in">
    <h2 class="text-3xl font-extrabold text-center text-indigo-700 mb-6">Lawyer Sign In</h2>

    <form method="POST" enctype="multipart/zform-data" class="space-y-6">
    ...
    <p class="text-center text-sm text-gray-600 mt-6">
      ...
      <a href="lawyersignup.php" class="text-indigo-600 font-medium hover:underline">Sign up here</a>
    </p>
  </div>
